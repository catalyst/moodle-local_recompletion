...
use local_recompletion\reportbuilder\entities\grades_archived;
use local_recompletion\reportbuilder\local\filters\user as user_filter;

/**
 * Archived choice answers report class implementation.
 *
 ...
 */
class archived_grades extends system_report {
    /**
    ...
    protected function initialise(): void {
        ...
        $gaentitiy = new grades_archived();
        $tablealias = $gaentitiy->get_table_alias('grade_grades_history');
        $this->add_entity($gaentitiy);

        $this->set_main_table('grade_grades_history', $tablealias);
        ...
        $garchivedalias = $gaentitiy->get_table_alias('local_recompletion_grade_archived');
        $join = "JOIN {local_recompletion_archived} {$archivedalias}
                   ON {$archivedalias}.userid = {$tablealias}.userid
                  AND {$archivedalias}.courseid = {$garchivedalias}.courseid
                  AND {$archivedalias}.timearchived = {$garchivedalias}.timearchived";
        ...
        if ($courseid) {
            ...
            $this->add_base_condition_sql("{$garchivedalias}.courseid = :{$paramcourseid}", [$paramcourseid => $courseid]);
        }
    ...
    public function add_columns(): void {
        $columns = [
            ...
            'grades_archived:item',
            'grades_archived:finalgrade',
            'grades_archived:rawgrade',
            'grades_archived:usermodified',
            'grades_archived:locktime',
            'grades_archived:overridden',
            'grades_archived:timemodified',
            'grades_archived:timearchived',
