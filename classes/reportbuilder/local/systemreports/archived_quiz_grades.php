...
/**
 * Archived quiz grades report class implementation.
 *
 ...
 */
class archived_quiz_grades extends system_report {
    /**
    ...
    protected function initialise(): void {
        $qgentitiy = new quiz_grades();
        $tablealias = $qgentitiy->get_table_alias('local_recompletion_qg');
        $this->add_entity($qgentitiy);

        $this->set_main_table('local_recompletion_qg', $tablealias);
    ...
    protected function add_filters(): void {
        ...
        if ($includefilters) {
            ...
            $qgentity = $this->get_entity('quiz_grades');
            $tablealias = $qgentity->get_table_alias('local_recompletion_qg');
            $this->add_filter((new filter(
                select::class,
                'quizid',
                new lang_string('pluginname', 'quiz'),
                $qgentity->get_entity_name(),
                "{$tablealias}.quiz"
            ))
                ->add_joins($this->get_joins())
