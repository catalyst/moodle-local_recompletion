...
use local_recompletion\reportbuilder\entities\lesson_overrides;
use local_recompletion\reportbuilder\helper;
use local_recompletion\reportbuilder\local\filters\user as user_filter;

/**
 * Archived lesson overrides report class implementation.
 *
 ...
 */
class archived_lesson_overrides extends system_report {
    /**
    ...
    protected function initialise(): void {
        ...
        $loentitiy = new lesson_overrides();
        $tablealias = $loentitiy->get_table_alias('local_recompletion_lo');
        $this->add_entity($loentitiy);

        $this->set_main_table('local_recompletion_lo', $tablealias);
    ...
    public function add_columns(): void {
        $columns = [
            ...
            'lesson_overrides:available',
            'lesson_overrides:deadline',
            'lesson_overrides:review',
            'lesson_overrides:maxattempts',
            'lesson_overrides:retake',
            'lesson_overrides:timearchived',
    ...
    protected function add_filters(): void {
        ...
        $loentity = $this->get_entity('lesson_overrides');
        $tablealias = $loentity->get_table_alias('local_recompletion_lo');
        $this->add_filter((new filter(
            select::class,
            'lessonid',
            new lang_string('pluginname', 'lesson'),
            $loentity->get_entity_name(),
            "{$tablealias}.lessonid"
        ))
            ->add_joins($this->get_joins())
