...
use local_recompletion\reportbuilder\entities\tool_certificate_issues;
use local_recompletion\reportbuilder\local\filters\user as user_filter;

/**
 * Archived course certificate issues report class implementation.
 *
 ...
 */
class archived_coursecertificate_issues extends system_report {
    /**
    ...
    public static function report_visisble(): bool {
        ...
        return array_key_exists('coursecertificate', $enabled);
    }
    ...
    protected function initialise(): void {
        ...
        $this->add_entity($tcientitiy);

        $this->set_main_table('tool_certificate_issues', $tablealias);
        ...
        $join = "JOIN {local_recompletion_archived} {$archivedalias}
                  ...
                  AND {$archivedalias}.courseid = {$tablealias}.courseid
                  AND {$archivedalias}.timearchived = {$tciarchivedalias}.timearchived";
        ...
        // Add regular clauses.
        $archived = database::generate_param_name('archived');
        $component = database::generate_param_name('component');
        $sql = "{$tablealias}.archived = :{$archived}
            AND {$tablealias}.component = :{$component}";
        $params = [
            $archived => 1,
    ...
    public function add_columns(): void {
        $columns = [
            ...
            'tool_certificate_issues:coursecertificate',
            'tool_certificate_issues:code',
            'tool_certificate_issues:templatename',
            'tool_certificate_issues:expirydate',
            'tool_certificate_issues:timearchived',
