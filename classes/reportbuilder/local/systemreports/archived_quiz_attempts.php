 ...
 */
class archived_quiz_attempts extends system_report {
    /**
    ...
    public static function report_visisble(): bool {
        ...
        return array_key_exists('quiz', $enabled);
    }
    ...
    protected function initialise(): void {
        $qaentitiy = new quiz_attempts();
        $tablealias = $qaentitiy->get_table_alias('local_recompletion_qa');
        $this->add_entity($qaentitiy);

        $this->set_main_table('local_recompletion_qa', $tablealias);
    ...
    protected function add_filters(): void {
        ...
        if ($includefilters) {
            ...
            $qaentity = $this->get_entity('quiz_attempts');
            $tablealias = $qaentity->get_table_alias('local_recompletion_qa');
            $this->add_filter((new filter(
                select::class,
                'quizid',
                new lang_string('pluginname', 'quiz'),
                $qaentity->get_entity_name(),
                "{$tablealias}.quiz"
            ))
                ->add_joins($this->get_joins())
                ->set_options(helper::get_available_cm_instances($courseid, 'quiz')));
