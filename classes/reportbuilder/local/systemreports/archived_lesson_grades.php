...
/**
 * Archived lesson grades report class implementation.
 *
 ...
 */
class archived_lesson_grades extends system_report {
    /**
    ...
    protected function initialise(): void {
        ...
        $lgentitiy = new lesson_grades();
        $tablealias = $lgentitiy->get_table_alias('local_recompletion_lg');
        $this->add_entity($lgentitiy);

        $this->set_main_table('local_recompletion_lg', $tablealias);
    ...
    protected function add_filters(): void {
        ...
        if ($includefilters) {
            ...
            $lgentity = $this->get_entity('lesson_grades');
            $tablealias = $lgentity->get_table_alias('local_recompletion_lg');
            $this->add_filter((new filter(
                select::class,
                'lessonid',
                new lang_string('pluginname', 'lesson'),
                $lgentity->get_entity_name(),
                "{$tablealias}.lessonid"
            ))
                ->add_joins($this->get_joins())
