...
use local_recompletion\reportbuilder\entities\questionnaire_responses;
 ...
 */
class archived_questionnaire_responses extends system_report {
    /**
    ...
    public static function report_visisble(): bool {
        ...
        return array_key_exists('questionnaire', $enabled);
    }
    ...
    protected function initialise(): void {
        ...
        $qrentitiy = new questionnaire_responses();
        $tablealias = $qrentitiy->get_table_alias('local_recompletion_qr');
        $this->add_entity($qrentitiy);

        $this->set_main_table('local_recompletion_qr', $tablealias);
    ...
    public function add_columns(): void {
        $columns = [
            ...
            'questionnaire_responses:question',
            'questionnaire_responses:response',
            'questionnaire_responses:status',
            'questionnaire_responses:grade',
            'questionnaire_responses:timearchived',
    ...
    protected function add_filters(): void {
        ...
        $qrentity = $this->get_entity('questionnaire_responses');
        $tablealias = $qrentity->get_table_alias('local_recompletion_qr');
        $this->add_filter((new filter(
            select::class,
            'questionnaireid',
            new lang_string('pluginname', 'questionnaire'),
            $qrentity->get_entity_name(),
            "{$tablealias}.questionnaireid"
        ))
            ->add_joins($this->get_joins())
            ->set_options(helper::get_available_cm_instances($courseid, 'questionnaire')));
    }
}
