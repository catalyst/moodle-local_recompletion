...
/**
 * Report builder entity for mod_lesson timer archived records.
 *
 ...
 */
class lesson_timers extends base {
    /**
    ...
    protected function get_default_entity_title(): lang_string {
        return new lang_string('entity:local_recompletion_lt', 'local_recompletion');
    }
    ...
    protected function get_all_columns(): array {
        $tablealias = $this->get_table_alias('local_recompletion_lt');
        ...
        $columns[] = (new column(
            'starttime',
            new lang_string('report:starttime', 'local_recompletion'),
            $this->get_entity_name()
        ))
            ->add_joins($this->get_joins())
            ->set_type(column::TYPE_TIMESTAMP)
            ->add_field("{$tablealias}.starttime")
            ->set_is_sortable(true)
            ->add_callback([format::class, 'userdate']);

        $columns[] = (new column(
            'endtime',
            new lang_string('report:endtime', 'local_recompletion'),
            $this->get_entity_name()
        ))
            ->add_joins($this->get_joins())
            ->set_type(column::TYPE_TIMESTAMP)
            ->add_field("{$tablealias}.lessontime")
            ->set_is_sortable(true)
            ->add_callback([format::class, 'userdate']);
        ...
        $columns[] = (new column(
            ...
            ->add_joins($this->get_joins())
            ->set_type(column::TYPE_INTEGER)
            ->add_field("{$tablealias}.completed")
            ->set_is_sortable(true)
            ->add_callback(static fn(int $value) => $value ? get_string('yes') : get_string('no'));

        $columns[] = (new column(
            'timemodifiedoffline',
            new lang_string('report:timemodifiedoffline', 'local_recompletion'),
            $this->get_entity_name()
        ))
            ->add_joins($this->get_joins())
            ->set_type(column::TYPE_INTEGER)
            ->add_field("{$tablealias}.timemodifiedoffline")
            ->set_is_sortable(true)
            ->add_callback([format::class, 'userdate']);
